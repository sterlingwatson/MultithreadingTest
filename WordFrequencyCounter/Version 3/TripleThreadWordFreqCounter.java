...
public class TripleThreadWordFreqCounter {
    ...
    private static boolean splittingFinished = false;
    ...
    public static class SplitingThread implements Runnable{        
    ...
    public void run(){
        ...
        splittingFinished = true;
    }
}


public static class CountingThread3 implements Runnable{        
    
    @Override
    public void run(){
        
        while (!splittingFinished || !sharedQueue2.isEmpty()){
            
            String word = sharedQueue2.poll();
            if (word != null){
                lengthCounter(word);
            }
    ...
    public static void RemoveSpecialChar(String string){
        string = string.replaceAll("[^a-zA-Z0-9\\-\\_]", " ");
        List<String> wordList = new ArrayList<String>(Arrays.asList(string.split((" "))));
        for (String s : wordList) {
            if (s.isEmpty()){
                continue;
            }else{
